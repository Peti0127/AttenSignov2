...
5. Sideload the included `manifest.xml` version `1.5.0.0`.
6. Restart Outlook.
...
Manifest version `1.5.0.0` includes both the mobile settings command and the mobile
...
Version `1.5.0.0` reads the current compose item’s From field. If its address
...
Signatures inserted by version `1.5.0.0` contain both an HTML attribute and a
